...
- `몸무게` 메뉴는 `건강`으로 확장되었고 식단/몸무게 탭을 제공합니다. 식단은 날짜와
  아침·점심·저녁·간식, 음식명, 선택적인 칼로리·단백질을 저장합니다.
- 사용자가 칼로리와 단백질을 모두 입력했거나 GPT가 추정한 음식은 DB에 영양정보를
  저장합니다. 같은 음식명을 다시 입력하면 저장된 값을 자동 완성합니다.
- `GPT로 평가하기`는 이번 주 미확정 영양값을 한 번에 추정한 뒤 다이어트 관점의 좋았던 점,
  가능한 한 피할 음식, 줄이기를 권하는 음식을 구분해 저장합니다. 식단이 바뀌면 기존
  평가는 무효화됩니다. 일요일에는 `주간 식단 평가`가 오늘 목록에 자동 표시됩니다.
- 오늘 목록의 `주간 식단 평가` 제목은 건강 탭을 열고, 원형 체크 영역은 평가 항목을 직접 완료하거나 다시 엽니다.
  이 체크 동작 수정은 로컬 검토 후 운영 배포 대상으로 승인되었습니다.
- BMI 카드에는 대한비만학회 성인 기준의 저체중·정상·비만 전단계·1/2/3단계 비만 분류를
  함께 표시합니다. BMI와 GPT 식단 평가는 참고 정보이며 의료 진단을 대신하지 않습니다.

2026-09-13 식단·BMI 분류 변경은 로컬 검토를 마쳤으며 통합 배포 대상으로 승인되었습니다.
기존 2026-09-12 누적 변경은 통합 배포 릴리스에 포함됐습니다. 로컬 테스트 몸무게 기록은
배포 전에 초기화했으며 키 176cm 설정은 유지했습니다.

## Render 무료 서버 시작 처리

운영 프런트엔드는 페이지를 열 때 먼저 자체 /api/health 경로를 통해
Render 백엔드의 /health를 확인합니다. 서버가 절전 상태이면
"서버를 준비하는 중입니다"를 표시하고 최대 3분 동안 자동 재시도합니다.
정상 응답을 받은 뒤에만 계획 데이터를 불러옵니다.

이 확인은 페이지를 열거나 사용자가 다시 시도할 때만 실행됩니다.
외부 keep-alive나 백그라운드 주기 호출은 사용하지 않으므로, 사용하지 않는
동안 Render 무료 서비스는 자연스럽게 절전 상태로 전환될 수 있습니다.