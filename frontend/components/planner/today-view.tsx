...
export function TodayView({ loading, items, plans, schedules, weekPlans, tasks, openAgent, openWeight, addItem, toggleItem, togglePlan, toggleSchedule, removeItem, reorderItems }: {
  ...
  function checklistItem(item: TodayItem, canDrag: boolean) {
    const isWeightPrompt = item.source === "RECURRING" && item.recurrence_id === null && item.title === "몸무게 기록"
    return <div key={item.id} onDragOver={(event) => canDrag && event.preventDefault()} onDrop={(event) => canDrag && dropItem(event, item.id)} className={`group flex min-h-14 items-center gap-2 border-b border-[#EDF0ED] py-2 transition last:border-b-0 ${draggedId === item.id ? "opacity-45" : ""}`}>
      ...
      <div className="min-w-0 flex-1"><p className={`break-words font-medium ${item.status === "COMPLETED" ? "text-[#8A928E] line-through" : "text-[#283B34]"}`}>{isWeightPrompt ? <button type="button" onClick={openWeight} className="rounded text-left underline decoration-[#9CB7AC] underline-offset-4 hover:text-[#285A4D] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5F8C7C]">{item.title}</button> : item.title}</p><div className="mt-1 flex flex-wrap gap-1.5">{item.source === "RECURRING" && <Badge variant="secondary" className="h-5 bg-[#EEF3F0] px-1.5 text-[0.68rem] text-[#53665E]"><Repeat2 className="size-3" />{isWeightPrompt ? "몸무게 탭 열기" : "반복"}</Badge>}{item.source === "TASK" && <Badge variant="secondary" className="h-5 bg-[#EEF3F0] px-1.5 text-[0.68rem] text-[#53665E]">할 일에서 추가</Badge>}{item.completion_target > 1 && <Badge variant="outline" className="h-5 px-1.5 text-[0.68rem]">{item.completion_count * 500}ml · {item.completion_count}/{item.completion_target}</Badge>}</div></div>
